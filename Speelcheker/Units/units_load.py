...
def get_units() -> pd.DataFrame:
    ...
    #
    # path = r"F:\Kazak\GoogleDrive\1_KK\Job_CNAC\SQLite\UnitsMeasurement"
...
def look_up_units(target: str, df: pd.DataFrame) -> str | None:
    for record in df.to_records(index=False):
        if target != record['abbreviation']:
            variants = [x.strip() for x in record['TARGET'].split(',')]
...
if __name__ == '__main__':
    ...
    raw = data["UNIT_OF_MEASURE"].unique().tolist()
    if None in raw:
        raw.remove(None)
    # print(len(raw), raw)
